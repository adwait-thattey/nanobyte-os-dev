...
void _cdecl x86_Video_WriteCharacterTeletype(char c, uint8_t page);

void _cdecl x86_div64_32(uint64_t dividend, uint32_t divisor, uint64_t* quotientOut, uint32_t* remainderOut);

// write functions to read from disk
...
// so that we can get drive details without relying on boot sector
